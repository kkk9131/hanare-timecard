...
export function runMigrations(dbPath) {
    ...
    try {
        ...
        const applied = new Set(db.prepare("SELECT name FROM __drizzle_migrations").all().map((r) => r.name));
        const files = listMigrationFiles();
        if (files.length === 0) {
            console.log("[migrate] no migration files found in drizzle/");
            return;
        }
        const insertApplied = db.prepare("INSERT INTO __drizzle_migrations (name, applied_at) VALUES (?, ?)");
        let appliedCount = 0;
