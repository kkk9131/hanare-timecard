...
2. 自分の氏名タイルをタップ
3. 4〜6 桁 PIN を入力 → 出勤 / 退勤 / 休憩開始 / 休憩終了 のいずれかをタップ
...
### 5.1 従業員 PIN (UI 経由 / 推奨)

1. 管理者で `/admin/employees` を開く
2. 対象従業員の「編集」
3. 「PIN リセット」→ 新しい 4〜6 桁を入力して保存
4. 口頭で本人に伝達 (印刷物で渡さない)
...
### 5.3 DB 直接操作 (最終手段、管理者パスワード紛失時)

```bash
# PIN ロック解除
sqlite3 data/hanare.db "UPDATE employees SET pin_fail_count=0, lock_until=NULL WHERE login_id='oyakata';"
...
店長 / 管理者の `/admin/employees` から対象従業員の「ロック解除」ボタンで即時解除できる。5 分待つ必要はない。
...
- PIN / パスワードは bcrypt でハッシュ化して保存される (平文非保持)
- PIN は誤入力 5 回で 5 分ロック
- 運用上の推奨: PIN は半年に 1 回ローテーション、退職者は即 `/admin/employees` で退職処理
