...
本番では、アクセス先を毎日変えないために **サーバー PC の IP アドレスとポート番号を固定**する。

```bash
PORT=3000 ./start.sh
```

例として、サーバー PC の固定 IP が `192.168.1.42` の場合、現場で使う URL は次の 1 つに統一する。

```text
http://192.168.1.42:3000
```

ポート番号を変える必要がある場合は `PORT=8080 ./start.sh` のように起動し、各端末のブックマークも `http://192.168.1.42:8080` にそろえる。
...
本番では、**打刻用 PC / iPad** と **管理者用 PC** の役割を分けて運用する。

```text
従業員用 PC / iPad
  -> http://<サーバーPCの固定IP>:3000/
  -> 出勤・退勤・休憩・修正申請

管理者用 PC
  -> http://<サーバーPCの固定IP>:3000/admin/login
  -> 従業員管理・シフト・承認・月次エクスポート

本番サーバー PC
  -> アプリ本体と data/hanare.db を保持
  -> 営業中は常時起動
```
...
| 端末 | 開く URL | 主な用途 |
| --- | --- | --- |
| 従業員用 PC / iPad | `http://<サーバーPCの固定IP>:3000/` | 打刻、従業員本人の確認、修正申請 |
| 管理者用 PC | `http://<サーバーPCの固定IP>:3000/admin/login` | 管理画面、承認、シフト、月次出力 |
| 本番サーバー PC | `http://localhost:3000` | 起動確認、保守、バックアップ確認 |

### 開店前の確認

1. 本番サーバー PC の電源が入っていることを確認する
2. `PORT=3000 ./start.sh`、または自動起動設定でアプリが起動していることを確認する
3. 従業員用 PC / iPad で `http://<サーバーPCの固定IP>:3000/` を開く
4. 本店・離れの店舗切り替えが正しく表示されることを確認する
5. 管理者用 PC で `/admin/login` が開けることを確認する

### 営業中の運用

- 従業員は従業員用 PC / iPad で打刻する
- 管理者・店長は管理者用 PC から `/admin/login` に入る
- 打刻漏れや誤打刻は、従業員が修正申請を出し、店長または管理者が承認する
- 従業員用 PC / iPad は店内 LAN の端末として扱い、外部の人が操作できない場所に置く

### 閉店後・日次確認

1. 管理者または店長が `/admin` で当日の出勤状況を確認する
2. 未処理の修正申請が残っていないか確認する
3. 必要に応じて `npm run backup` を実行し、`data/backups/` にバックアップが作られたことを確認する
4. 本番サーバー PC は翌営業日に備えて、スリープしない設定を維持する

### ネットワーク運用ルール

- 店内の必要な端末だけが `http://<サーバーPCの固定IP>:3000` にアクセスできるようにする
- ルーターのポート開放で、インターネットから 3000 番ポートへ入れる設定にしない
- お客様用 Wi-Fi やゲスト Wi-Fi から本番サーバー PC にアクセスできないようにする
- サーバー PC の IP アドレスを変更した場合は、従業員用 PC / iPad と管理者用 PC のブックマークを更新する

### 止まったときの一次対応

1. 従業員用 PC / iPad で画面を再読み込みする
2. 本番サーバー PC で `http://localhost:3000` が開くか確認する
3. 開かない場合は、本番サーバー PC で `PORT=3000 ./start.sh` を実行する
4. `EADDRINUSE` が出る場合は、すでに起動中の可能性があるため、担当者がポート使用状況を確認する
5. DB やデータ破損が疑われる場合は、先に `data/hanare.db` と `data/backups/` をコピーして保全し、復旧手順に進む
...
- [ ] 本番サーバー PC を 1 台に決めた
- [ ] 本番サーバー PC の IP アドレスを固定した
- [ ] 本番ポートを `3000` など 1 つに決め、各端末の URL を統一した
- [ ] 本店・離れの端末から同じ本番サーバー PC にアクセスできる
- [ ] 従業員用 PC / iPad と管理者用 PC の役割を分けた
- [ ] 管理者・店長の初期パスワードを変更した
- [ ] 従業員マスタと所属店舗を確認した
