...
iPad で `http://<サーバ IP>:3000/` を開くとキオスク画面が表示される。店舗を選び、出勤者一覧から自分の氏名をタップすると、そのまま「出勤」「退勤」「休憩開始」「休憩終了」の打刻画面へ進む。状況に応じて 1 つだけタップすると打刻が完了し、数秒後に氏名選択画面へ自動で戻る。誤って打刻した場合は、従業員ホーム `/me` から修正申請を作成し、店長/管理者の承認を待つ。
